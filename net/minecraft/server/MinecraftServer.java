...
public class MinecraftServer implements ICommandListener, Runnable {
    ...
    private void c(String s) {
        ...
        for (int i = -b1; i <= b1; ++i) {
            this.a("Preparing spawn area", (i + b1) * 100 / (b1 + b1 + 1));
                    ...
                    if (s.toLowerCase().startsWith("op ")) {
                        ...
                        if (s.toLowerCase().startsWith("ban ")) {
                            ...
                            for (int i = 0; i < this.f.b.size(); ++i) {
                                EntityPlayerMP entityplayermp1 = (EntityPlayerMP) this.f.b.get(i);

                                if (entityplayermp1.ar.equalsIgnoreCase(s2)) {
                                    entityplayermp = entityplayermp1;
                                }
                            }

                            if (entityplayermp != null) {
                                entityplayermp.a.c("Kicked by admin");
                                this.a(s1, "Kicking " + entityplayermp.ar);
                            } else {
                            ...
                            if (s.toLowerCase().startsWith("tp ")) {
                                astring = s.split(" ");
                                if (astring.length == 3) {
                                    entityplayermp = this.f.h(astring[1]);
                                    entityplayermp2 = this.f.h(astring[2]);
                                    if (entityplayermp == null) {
                                        ...
                                        entityplayermp.a.a(entityplayermp2.p, entityplayermp2.q, entityplayermp2.r, entityplayermp2.v, entityplayermp2.w);
                                ...
                                entityplayermp2 = this.f.h(s3);
                                if (entityplayermp2 != null) {
                                    try {
                                        int j = Integer.parseInt(astring[2]);

                                        if (Item.c[j] != null) {
                                            this.a(s1, "Giving " + entityplayermp2.ar + " some " + j);
                                            int k = 1;
                                            ...
                                            entityplayermp2.a(new ItemStack(j, k));
                                        } else {
